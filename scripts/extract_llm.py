...
def extract_yutai_text(doc):
    """Parse Yutai selection table from raw text (find_tables merges cells incorrectly)."""
    ...
    # Find the header row to know column mapping
    header_seen = False
    for i, line in enumerate(lines):
        stripped = line.strip()
        if not stripped:
            continue
        if not header_seen and "简介" in stripped and "封装" in stripped:
            header_seen = True
            continue
        if not header_seen:
            continue
        
        # Series header (e.g., "80系列/车规", "88系列PHY", "85系列/phy系列")
        if re.match(r'^[\d]+系列', stripped) or ("系列" in stripped and len(stripped) <= 20):
            current_series = stripped
            continue
        
        # Product row: starts with a part number
        if is_part_number(stripped):
            part = stripped
            # Collect up to 10 following lines for parameters
            params = []
            j = i + 1
            while j < len(lines) and len(params) < 10:
                nl = lines[j].strip()
                if nl and (is_part_number(nl) or ("系列" in nl and len(nl) <= 20)):
                    break  # next product or next series
                if nl and len(nl) < 80:  # legitimate param line
                    params.append(nl)
                j += 1
            
            # Label params with schema
            labeled = []
...
def is_part_number(s):
    ...
    # Blacklist: common words, package codes, signal names
    blacklist = {"can","lin","sbc","phy","dcdc","adc","dac","ldo","pmic","part",
                 "收发器","传感器","隔离器","驱动器","放大器","比较器","转换器","开关","运放","网卡","交换机","none"}
    if s.lower() in blacklist: return False
    
    # Package codes: SOT23-5, QFN32, SOP8, WSOP16, DFN3X3-8, etc.
    pkg_patterns = ['SOT','QFN','DFN','SOP','SOIC','SSOP','MSOP','TSSOP','SSOP',
                    'WSOP','EMSOP','ESOP','WLCSP','LQFP','HLQFP','VQFN','SC70',
                    'HSSOP','HMSOP','HSOP','HTSSOP','ETSSOP','BGA','CSP','LGA',
                    'TO247','TO220','TO252','TO263','SMP8']
    for pp in pkg_patterns:
        idx = s.upper().find(pp)
        if idx >= 0:
            before = s[:idx]
            # If prefix is short (0-1 chars) -> pure package code
            # But preserve manufacturer prefixes like "NSOPA" (Novosense), "EMSOP8" (this IS a package)
            if len(before) <= 1:
                return False
            # Also reject if the prefix looks like a number+letter pattern (e.g., "3PD9708E")
            if re.match(r'^\d', before):
                return False
    
    # Signal/acronym names that pass the regex but aren't products
    signal_names = {'VCC','GND','VDD','VIN','TXD','RXD','SCK','SDA','SCL','CSB',
                    'MCU','DSP','BMS','ECU','PWM','SPI','I2C','ESD','YES','INL',
                    'PSM','PSR','TTL','OTP','PGA','SAR','IIR','MUX','AMP','PC H',
                    'OUT','EN1','INA','INF','SEL','FET','PRE','OOK','GH1','COT',
                    'PCM','ICL','FS1','ADB','SSN','BST','PMU','EXC','AOP','RDI',
                    'ISO','OWI','OSC','STA','ITA','VBG','VCP','VLD','VIP','BAT',
                    'INH','STB','DIS','WPN','TRX','PLC','S2P','ABZ','POR','PDU',
                    'OBC','SD6','ID0','SC3','SDO','MSL'}
    if s.upper() in signal_names:
        return False
    
    # Must start with letter, have enough length, contain at least one digit
...
def extract_raw(doc, vendor_name):
    # Yutai: table extraction merges cells incorrectly — use raw-text parser
    if "裕太" in vendor_name:
        return extract_yutai_text(doc)
    products = []
    for pn in range(len(doc)):
        ...
        section_hints = []
        for kw in ["CAN","LIN","收发器","放大器","运放","比较器","隔离","电源","传感器","开关","PHY","以太网","马达","驱动","ADC","DAC","接口","DCDC","LDO","SBC","RS-485","I2C","栅极","基准","复位","网卡","交换机"]:
...
标签词汇表（只能从中选择）：
车规AEC-Q100 | 工业级 | 消费级 | CAN FD | 特定帧唤醒(Partial Networking) | 低功耗唤醒 | LIN | VIO | 高耐压 | 轨到轨 | 高速(≥50MHz) | 中速(≥10MHz) | 超低功耗(≤1µA) | 低功耗(≤50µA) | 精密(≤1mV Vos) | 高压(≥30V) | 千兆 | 2.5G | 百兆 | 100FX | T1 PHY | Pin-to-Pin兼容 | 5kVrms隔离 | 3kVrms隔离 | 隔离电源 | 电流传感器 | 温度传感器 | 压力传感器 | 位置传感器
...
- 100FX/光纤/fiber→100FX (以太网PHY支持100FX光纤模式时加)
- T1/100Base-T1/1000Base-T1/802.3bw/802.3bp→T1 PHY (车载单对以太网PHY)
